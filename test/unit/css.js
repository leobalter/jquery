...
QUnit.test( ":visible/:hidden selectors", function( assert ) {
	...
	assert.ok( !jQuery("#siblingspan").is(":visible"), "Span with no content not visible (#13132)" );
	$newDiv = jQuery("<div><span></span></div>").appendTo("#qunit-fixture");
	assert.equal( $newDiv.find(":visible").length, 0, "Span with no content not visible (#13132)" );
	$br = jQuery("<br/>").appendTo("#qunit-fixture");
	assert.ok( !$br.is(":visible"), "br element not visible (#10406)");
